...
def _feature_paths(cfg: FullAircraftConfig, split: str) -> dict[str, Path]:
    return {
...
def _stats_from_builder(x_path: Path, builder, batch_rows: int = 400_000) -> tuple[np.ndarray, np.ndarray]:
    ...
    for start in range(0, x_raw.shape[0], batch_rows):
        ...
        feats = builder(np.asarray(x_raw[start:end], dtype=np.float32))
...
def _transform_split(
    ...
    x_path, y_path = reduced_paths(cfg, split)
    x_raw = np.load(x_path, mmap_mode="r")
    y_raw = np.load(y_path, mmap_mode="r")
    paths = _feature_paths(cfg, split)

    n_rows = x_raw.shape[0]
    expert_example = build_expert_features(np.asarray(x_raw[:1], dtype=np.float32))
    gate_example = build_encoder_features(np.asarray(x_raw[:1], dtype=np.float32))
    ...
    for start in range(0, n_rows, batch_rows):
        ...
        x_chunk = np.asarray(x_raw[start:end], dtype=np.float32)
        y_chunk = np.asarray(y_raw[start:end, [cfg.cp_column]], dtype=np.float32)
...
def prepare_feature_store(cfg: FullAircraftConfig) -> None:
    cfg.ensure_dirs()

    train_x_path, train_y_path = reduced_paths(cfg, "train")
    if not train_x_path.exists() or not train_y_path.exists():
        raise FileNotFoundError("Reduced data not found. Run 'prepare-reduced-data' first.")

    expert_mean, expert_scale = _stats_from_builder(train_x_path, build_expert_features)
    gate_mean, gate_scale = _stats_from_builder(train_x_path, build_encoder_features)
